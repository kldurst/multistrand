...
#define getBoolAttr(obj, name, pvar) _m_getAttr_DECREF( obj, #name, PyLong_AsLong, pvar, bool)
#define getLongAttr(obj, name, pvar) _m_getAttr_DECREF( obj, #name, PyLong_AsLong, pvar, long)
#define getDoubleAttr(obj, name, pvar) _m_getAttr_DECREF( obj, #name, PyFloat_AS_DOUBLE, pvar, double)

// Accessors (borrowed refs only)
#define getLongItem(list, index) PyLong_AsLong(PyList_GET_ITEM(list, index))
#define getLongItemFromTuple(tuple, index) PyLong_AsLong(PyTuple_GET_ITEM(tuple, index))
...
#define getLongItem(list, index) \
  (PyInt_Check(PyList_GET_ITEM(list, index))?PyLong_AsLong(PyList_GET_ITEM(list,index)):-1)

#define getLongItemFromTuple(tuple, index) \
  (PyInt_Check(PyTuple_GET_ITEM(tuple, index))?PyLong_AsLong(PyTuple_GET_ITEM(tuple,index)):-1)
...
static inline bool _m_testLongAttr(PyObject *obj, const char *attrname, const char *test, long value) {
	...
	long local_val = PyLong_AsLong(_m_attr);
	Py_DECREF(_m_attr);
	if (test[0] == '=')
...
static inline bool _m_d_testLongAttr( PyObject *obj, const char *attrname, const char *test, long value )
{
 ...
 but the returned object from GetAttrString was still NULL!\n");
	...
	else
	{
		...
		local_val = PyLong_AsLong(_m_attr);
		Py_DECREF(_m_attr);
		if( test[0] == '=' )
		return (local_val == value);
		if( test[0] == '<' )
